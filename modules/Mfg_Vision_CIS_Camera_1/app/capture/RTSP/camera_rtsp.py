...
class RTSP_Camera():
    ...
    def __init__(self, camID, camTrigger, camURI, camLocation, camPosition, camFPS, inferenceFPS, modelACV,
        ...
        # self.safety_boundary = [(235, 461), (237, 377), (239, 300), (246, 229), (250, 201), (394, 196), (404, 248), (417, 360), (424, 457), (238, 463)]
        self.safety_boundary = [(241, 326), (236, 244), (239, 172), (247, 95), (248, 59), (395, 55), (406, 113), (413, 180), (421, 247), (425, 317), (241, 326)]
        self.cap_RTSP()
            ...
            if _ == True:
                ...
                if self.inferenceFPS > 0:
                    if self.frameRateCount == int(self.camFPS/self.inferenceFPS):
                        self.frameRateCount = 0
                        h, w = frame.shape[:2]
                        frame_class = frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                        frame_class = frame_resize(img = frame_class, target = 224, model = "classification")
                        class_result = predict_class_multi_class(frame_class)
                        class_predictions = class_result['predictions']
                        class_t_infer = class_result["inference_time"]
                        print(f"Classification inference time: {class_t_infer}ms")


                        if class_predictions[0]['labelName'] == 'UP':
                            if self.modelACV:
                            ...
                            if detection_count > 0:
                                ...
                                safety_polygon = Polygon(self.safety_boundary)
                                object_poly_list = []                          
                                ...
                                for i in range(detection_count):
                                    ...
                                    # /////////////////////////////////////
                                # Object Detection with Boundary Area Identification
                                #
                                image_text = f"{probability}%"
                                color1 = (0, 0, 255)
                                color2 = (0, 255, 0)
                                ...
                                if bounding_box:
                                    if self.modelACV:
                                        ...
                                        start_point = (xmin, ymin)
                                        end_point = (xmax, ymax)
                                        if tag_name == "PERSON" or tag_name == "PALLET":
                                            annotated_frame = cv2.rectangle(annotated_frame, start_point, end_point, color1, thickness1)
                                            annotated_frame = cv2.putText(annotated_frame, image_text, start_point, fontFace = cv2.FONT_HERSHEY_TRIPLEX, fontScale = .4, color = (0,0,255))
                                        else:
                                            annotated_frame = cv2.rectangle(annotated_frame, start_point, end_point, color2, thickness2)
                                        ...
                                        if boundary_active:
                                            ...
                                            if object_polygon.intersects(safety_polygon):
                                            ...
                                            start_point = (int(bounding_box["left"]), int(bounding_box["top"]))
                                            end_point = (int(bounding_box["width"]), int(bounding_box["height"]))
                                            if tag_name == "PALLET" or tag_name == "PERSON":
                                                annotated_frame = cv2.rectangle(annotated_frame, start_point, end_point, color1, thickness1)
                                                annotated_frame = cv2.putText(annotated_frame, image_text, start_point, fontFace = cv2.FONT_HERSHEY_TRIPLEX, fontScale = .4, color = (0,0,255))
                                            else:
                                                annotated_frame = cv2.rectangle(annotated_frame, start_point, end_point, color2, thickness2)
                                    ...
                                    if len(object_poly_list) > 0:
                                        cv2.polylines(annotated_frame, np.array([self.safety_boundary]), False, poly_yellow, 3)
